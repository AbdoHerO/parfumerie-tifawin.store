...
class ReportController extends Controller
{
    ...
    public function stock_report(Request $request)
    {
        ...
        if ($request->clear_filter == null){
            if ($request->has('category_id')){
                $sort_by = $request->category_id;
                $products = $products->where('category_id', $sort_by);
            }
            if ($request->has('product_name')){
                $sort_by_name = $request->product_name;
                $products = $products->where('name', 'LIKE', '%' . Str::lower($sort_by_name) . '%');
            }
        }

        $products = $products->paginate(15);
        return view('backend.reports.stock_report', compact('products','sort_by','sort_by_name'));
    }
    ...
    public function in_house_sale_report(Request $request)
    {
        ...
        $products = $products->paginate(15);
        return view('backend.reports.in_house_sale_report', compact('products','sort_by','sort_by_name'));
    }
    ...
    public function wish_report(Request $request)
    {
        ...
        return view('backend.reports.wish_report', compact('products','sort_by','sort_by_name'));
    }
