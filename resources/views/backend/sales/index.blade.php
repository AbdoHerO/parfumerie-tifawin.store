...
<div class="card">
    <div class="card-header row gutters-5">
        <div class="col text-center">
            <h5 class="mb-md-0 h6">{{ translate('proceeds_from_orders') }}</h5>
            <span class="badge badge-inline badge-info">{{ translate('orders_paid_and_delivered') }}</span>
        </div>
        <div class="col-lg-3 text-center">
            <h6 class="mb-md-2 h6">{{ translate('total_gain') }}</h6>
            <span class="badge badge-inline badge-success">{{ single_price($total_gain) }}</span>
        </div>
        <div class="col-lg-3 text-center">
            <h6 class="mb-md-2 h6">{{ translate('total_achat') }}</h6>
            <span class="badge badge-inline badge-success">{{ single_price($total_achat) }}</span>
        </div>
        <div class="col-lg-3 text-center">
            <h6 class="mb-md-2 h6">{{ translate('total_global') }}</h6>
            <span class="badge badge-inline badge-success">{{ single_price($total_global) }}</span>
        </div>
    </div>
        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-lg-2 ml-auto">
                <select class="form-control aiz-selectpicker" name="delivery_status" id="delivery_status">
                    ...
                    <option value="returned" @if ($delivery_status == 'returned') selected @endif><!-- {{ translate('Returned') }} --> Retourné</option>
                    <option value="no_response" @if ($delivery_status == 'no_response') selected @endif><!-- {{ translate('No Response') }} --> Pas de réponse</option>
                    <option value="boite_vocal" @if ($delivery_status == 'boite_vocal') selected @endif><!-- {{ translate('Boite Vocal') }} --> Boite Vocal</option>
                </select>
            </div>
            <div class="col-lg-2 ml-auto">
                        ...
                        <td>
                            @if ($order->user != null)
                                @if ($order->user->user_type == "guest")
                                    ...
                                    <span class="d-block w-50 badge badge-inline badge-warning">{{translate('guest')}}</span>
                                @else
                                    {{ $order->user->name }}
                                @endif
                            @else
